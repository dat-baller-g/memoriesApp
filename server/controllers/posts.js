...
export const updatePost = async(req, res) =>{
    ...
    if(!mongoose.Types.ObjectId.isValid(_id)) return res.status(404).json("No post with such id")
...
export const deletePost = async(req, res) => {
    const {id} = req.params
    if(!mongoose.Types.ObjectId.isValid(id)) return res.status(404).json("No post with such id")
    console.log(id)
    try {
